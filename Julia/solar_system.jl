...
R[:, 1, :] = [
    ...
    1.52 0;
    0.72 0] 
V[:, 1, 2] = map(x -> x > 0 ? sqrt(G * M/x) : 0, R[:, 1, 1])
...
function animate_solver(solver, R, G, m, t, V, Δt, labels)
    for time = 1:(t - 1)
    ...
    anim = @animate for t = 1:size(R, 2)
        ...
        scatter(Rx', Ry', xlims = (-2.5, 2.5), ylims = (-2.5, 2.5), label = labels)
    end every 2
...
for solver in [verlet]
    animate_solver(solver, R, G, m, t, V, Δt, ["Sun", "Earth", "Mercury", "Mars", "Venus"])
end
